...
def main():
    if len(sys.argv) < 2:
        print("Usage: python3 model_multi_dataset_evaluation.py <path_to_models> <model_name_with_{run}> "
              "<cuda_device>\n"
              "Example: python3 model_multi_dataset_evaluation.py path/resources/taggers "
              "conll_eng_dutch_ner_roberta_large_run_{run}_BitFit 2")
        return
